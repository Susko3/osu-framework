    ...
    public class BenchmarkButtonInput
    {
        ...
        public KeyboardKeyInput FromTwoStates(int count)
        {
            ...
            for (int i = 0; i < count; i++)
                state1.SetPressed(KeyboardKey.FromKey((Key)i), true);

            for (int i = count; i < 2 * count; i++)
                state2.SetPressed(KeyboardKey.FromKey((Key)i), true);
        ...
        public InputState Apply()
        {
            ...
            for (int i = 0; i < 10; i++)
                entries.Add(new ButtonInputEntry<KeyboardKey>(KeyboardKey.FromKey((Key)i), true));
